...
function removeMasonryItemsByNumbers(pageHtml, g, numbers) {
    ...
    numbers.forEach(function (num) {
        const name = formatPhotoName(num, g.pad);
        const re = new RegExp(
            '<div class="masonry-item"[^>]*>[\\s\\S]*?thumbs/' +
                name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') +
                '\\.webp[\\s\\S]*?</div>\\s*',
            'i'
        );
        if (!re.test(html)) {
            throw new Error('Foto ' + name + ' non trovata nell\'HTML della gallery');
        }
...
/**
 * Sostituisce tutti i masonry-item con un nuovo elenco (ordine = numbersSorted)
 */
function replaceAllMasonryItems(pageHtml, g, numbersSorted) {
    ...
    const blockStart = pageHtml.lastIndexOf('<div', firstStart);
    const blockEnd = pageHtml.indexOf('</div>', lastStart);
    if (blockStart === -1 || blockEnd === -1) {
        throw new Error('HTML gallery non valido');
    }
    const insertEnd = blockEnd + '</div>'.length;
    const itemsHtml = numbersSorted
        .map(function (num) {
            return buildMasonryItem(g, num, num - 1);
        })
        .join('\n');
    return pageHtml.slice(0, blockStart) + itemsHtml + pageHtml.slice(insertEnd);
}
